...
def authorize_application(
    ...
    return render(
        request=request,
        template_name='Facebook/authorize_application.html',
        dictionary={
            'url': 'https://www.facebook.com/dialog/oauth?%s' % urlparse(query)
        },
        status=401
    )
...
def authorization_denied(request):
    ...
    return render(
        request=request,
        template_name='Facebook/authorization_denied.html',
        status=403
    )
